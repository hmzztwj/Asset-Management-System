...
- **逾期主动提醒**：进入系统时弹窗提醒一次（列超期最久的 5 项 + 一键去处理，同一会话当天内不再打扰）；首页有完整逾期清单卡片（含超期天数）；侧栏「资产领用」带红色数量徽标。
  **不做**每个页面正文上的常驻横幅 —— 提醒只在仪表盘和弹窗里出现
...
- 二维码里写的地址可用 `ASSETS_PUBLIC_BASE_URL` 固定（不配则跟随打印标签时的访问地址；用 `127.0.0.1` 打开标签页会印出本机地址、手机扫不开，标签页顶部会给出告警）
...
| `ASSETS_PUBLIC_BASE_URL` | 空（跟随当前访问地址） | 二维码里写哪个地址。留空时若用 `127.0.0.1` 打开标签页，标签会指向 `127.0.0.1`、手机扫不开；建议固定成 `http://192.168.1.50:12036` 或 `http://www.mdassetsmg.com` |
